        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className='space-y-4'>
            {orders.map((order) => (
                <div key={order.id} className='bg-white shadow-md rounded-lg p-4 border space-y-2'>
                    ...
                    <div className='flex justify-center '>
                        <button type='button' className='w-full md:w-1/2 mt-4 bg-just-red text-sm text-white py-2 rounded-md hover:bg-red-700 transition-colors'>
                            Mark as Fulfilled
                        </button>
                    </div>
                </div>
...
export default MobileNewOrders;
