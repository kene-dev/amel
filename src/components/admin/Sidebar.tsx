...
function Sidebar() {
    return (
        <nav className='hidden xl:block py-8 h-[98%] bg-white w-[95%] px-10 mx-4 rounded shadow-sm'>
            <span className='flex justify-center items-center'>
                <Link to='/admin'>
                    <Logo />
                </Link>
            </span>
            <ul className='mt-8 mx-auto w-fit flex flex-col gap-2 items-start'>
                {links.map((link) => {
                    ...
                    return (
                        <li key={path} className='relative w-full group'>
                            <NavLink
                                className={({ isActive }) =>
                                    `relative flex items-center gap-4 w-full px-6 py-1 rounded-md capitalize text-lg transition-all duration-300 ease-in-out ${
