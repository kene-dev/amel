...
import { useCreateJobsMutation } from '@/features/jobs/jobsApiSlice';
...
type createJobTypeInput = z.infer<typeof createJobSchema>;


const CreateJob: React.FC<CreateJobProps> = ({action})  => {
  ...
  const {register, handleSubmit, setValue, reset, formState:{errors, isSubmitting} } = useForm<createJobTypeInput>({
...
const formSubmit: SubmitHandler<createJobTypeInput> = async (data) => {
  ...
  return (
    ...
    {isSubmitting && <Loader />}
    {/* CREATE A JOB FORM */}
    <form onSubmit={handleSubmit(formSubmit)} className='w-full h-screen overflow-y-scroll flex flex-col gap-10 bg-white rounded-md p-4'>
    ...
    <div className='w-full flex items-start justify-between'>
      ...
      <div onClick={() => action(ViewType.ALL_JOBS)}>
      ...
      <AnimatePresence mode='sync'>
        ...
        <div className='w-full flex flex-col gap-1'>
            ...
            <div className='w-full flex items-center justify-between gap-3'>
                <div className=' h-[50px] flex flex-col gap-3 items-start relative border-[2px] border-black/60 rounded-md'>
        ...
        <div className='w-full'>
         <Button width='100%' type='submit' theme='red' textStyle='text-xs p-1 cursor-pointer' text="Post Job" />
        </div>
