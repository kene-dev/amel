...
const SingleJobView = ({action, ID}: SingleJobProps) => {
  ...
  <div className='lg:w-screen'>
    ...
    className="text-xl w-max flex items-center gap-5 mb-6 cursor-pointer"> 
        <FaArrowLeftLong className='text-black text-lg' />
        Back to all Job posts
    </motion.h1>
    {data && (
        <div key={data._id} className='lg:w-[668px] w-full flex flex-col gap-6'>
            {/* JOB IMAGE */}
            ...
            <div className='w-full flex flex-col gap-4 lg:flex-row items-start justify-between'>
                {/* JOB TITLE */}
                ...
                {/* JOB RENUMERATION */}
                <p className='text-sm text-black/80'>N{data.priceRangeMin} - N{data.priceRangeMax}/mon</p>
            </div>

            {/* JOB DESCRIPTION */}
            <div className='w-full'>
                <h3 className='font-semibold'>Description</h3>
                <div className="text-sm h-full text-black/90 lg:w-[640px] w-full job-description" dangerouslySetInnerHTML={{__html: sanitizeDescription(data.description)}}/>
            </div>
        </div>
    )}
  </div>
